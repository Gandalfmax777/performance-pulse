...
- **`useRankingStream` invalida 6 query keys** em cascata quando recebe `ranking:update` (lista em `src/hooks/useRankingStream.ts:54-61`). Se adicionar nova entidade que muda quando métrica é registrada, editar essa lista — senão a TV/dashboard ficam até 30s desatualizados. A invalidação de `["reports"]` é por prefixo, então cobre overview, kpi-series E `active-period` (`["reports","active-period",...]`).
- **Modo TV resolve a "semana ativa" pelo backend** (`useActiveTvPeriod` em `src/hooks/useReports.ts`, endpoint público `/api/reports/active-period`): `TvSlides.tsx` (`useTvSlideData`) usa a semana/mês da métrica mais recente do tenant, não a semana corrente fixa. Evita a TV zerar no início da semana (segunda de manhã, antes do 1º lançamento) e "vira" pra semana corrente via SSE ao 1º registro. `rangeForPeriod()` continua como fallback enquanto o endpoint carrega; o label do header sai de `data.rangeLabel`. Passa `{ start: range.from }` pros hooks `useWeeklyRanking`/`useMonthlyRanking` (já aceitam) pra manter ranking e overview no mesmo período.
- **`AdminGoals` — período da meta tem armadilha**: o alvo agregado do time é `valor × período × nº assessores`; meta **Diária** multiplica por 7 dias (prospecção 30 DAILY virava 2940 em vez de ~300). O form de **criar KPI** default era `DAILY` (≠ form de editar, `WEEKLY`) — corrigido pra `WEEKLY` em 2026-06-01. Os dois forms agora mostram `<GoalTargetPreview>` (espelha `computeAggregatedTarget` do backend) com aviso quando Diária. Período é escolha do admin — nada hardcoded.
