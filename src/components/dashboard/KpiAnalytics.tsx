...
const generateInsights = (assessors: Assessor[], scope: Scope, selectedId: string | null): string[] => {
  ...
  if (avgLeads < 8) insights.push(`📉 Geração de leads está ${Math.round(((10 - avgLeads) / 10) * 100)}% abaixo da meta. Priorize a lista de prospecção na segunda-feira.`);
  else insights.push(`✅ Leads acima de 80% da meta (${avgLeads.toFixed(0)}/10). Bom ritmo!`);

  if (avgCad < 60) insights.push(`⚠️ Cadência média de ${avgCad.toFixed(0)}% está crítica. Revise o processo de follow-up.`);
  else if (avgCad < 80) insights.push(`🔄 Cadência em ${avgCad.toFixed(0)}%. Próximo de bater meta, intensifique contatos.`);

  if (avgLig < 20) insights.push(`📞 Volume de ligações baixo (${avgLig.toFixed(0)}/30). Considere sessões extras de prospecção.`);

  if (avgReun < 2) insights.push(`🗓️ Reuniões agendadas insuficientes (${avgReun.toFixed(1)}/3). Foco em converter ligações em reuniões.`);
  else insights.push(`🎯 Meta de reuniões sendo atingida (${avgReun.toFixed(1)}/3).`);

  if (avgInd < 3) insights.push(`🤝 Indicações abaixo do esperado. Aproveite quarta-feira para fortalecer networking.`);

  if (avgBol < 7) insights.push(`💰 Boletas abaixo da meta (${avgBol.toFixed(0)}/10). Revise pipeline de quinta-feira.`);

  // Conversion rate insight
  if (avgLeads > 0) {
    const convRate = (avgReun / avgLeads) * 100;
    insights.push(`📊 Taxa de conversão Leads → Reuniões: ${convRate.toFixed(0)}%. ${convRate < 20 ? "Oportunidade de melhora na qualificação." : "Boa taxa de conversão!"}`);
  }
  ...
  if (scope === "geral" && target.length > 1) {
    ...
    insights.push(`🏆 Melhor desempenho: ${best.name} (${best.weeklyGoalPercent}%). Pior: ${worst.name} (${worst.weeklyGoalPercent}%).`);
  }
...
const KpiAnalytics = () => {
  ...
  // Multiplier for period simulation
  ...
  const barData = useMemo(() => {
    return (Object.keys(KPI_META) as KpiKey[]).map(key => {
      ...
      return {
        name: meta.label,
        Resultado: Math.round(avg * periodMultiplier),
        Meta: Math.round(meta.target * periodMultiplier),
      };
    });
  ...
  const radarData = useMemo(() => {
    return (Object.keys(KPI_META) as KpiKey[]).map(key => {
      ...
      return {
        kpi: meta.label,
        score: Math.min(100, Math.round((avg / meta.target) * 100)),
        fullMark: 100,
      };
    });
  }, [filteredAssessors]);
  ...
  const insights = useMemo(
    () => generateInsights(ASSESSORS, scope, selectedAssessor),
    [scope, selectedAssessor]
  );
    ...
    <div className="space-y-4">
      ...
      <div className="card-glass rounded-xl p-4">
        <div className="flex items-center flex-wrap gap-3">
          ...
          <div className="flex gap-1 bg-muted/20 rounded-lg p-1">
            <button
              onClick={() => { setScope("geral"); setSelectedAssessor(null); }}
              className={`px-3 py-1.5 rounded-md text-xs font-semibold flex items-center gap-1 transition-all ${
                scope === "geral" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-semibold flex items-center gap-1 transition-all ${
                scope === "individual" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"
              }`}
            >
        ...
        <div className="card-glass rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-bold text-foreground">
              KPIs vs Meta – {period === "dia" ? "Dia" : period === "semana" ? "Semana" : "Mês"}
            </h3>
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className={`text-sm font-mono font-bold ${overallPct >= 80 ? "text-success" : overallPct >= 50 ? "text-chart-orange" : "text-destructive"}`}>
                    {overallPct}%
                  </span>
                </div>
                  ...
                  {(Object.keys(KPI_META) as KpiKey[]).map(key => {
                    ...
                    return (
                      <div key={key} className="flex items-center justify-between px-1.5 py-0.5 rounded bg-muted/30">
                        ...
                        <span className={`font-mono font-semibold ${pct >= 80 ? "text-success" : pct >= 50 ? "text-chart-orange" : "text-destructive"}`}>
                          {val}{meta.unit}/{meta.target}{meta.unit}
                        </span>
                      </div>
                    );
                  })}
