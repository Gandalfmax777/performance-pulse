...
const DailyResults = () => {
  // Simulated daily score using existing KPI data
  ...
  const podiumColors = [
    ...
    "from-gold/30 to-gold/10 border-gold/40",
    "from-bronze/30 to-bronze/10 border-bronze/40",
  ];
  const podiumTextColors = ["text-silver", "text-gold", "text-bronze"];
        ...
        <div className="flex items-end justify-center gap-4 pt-8 pb-4">
          {podiumOrder.map((a, i) => {
            ...
            return (
              ...
              >
                {/* Avatar */}
                <div className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-lg border-2 mb-2 ${LEVEL_COLORS[a.level]}`}>
                  {a.avatar}
                </div>
                <p className="text-sm font-semibold text-foreground mb-1">{a.name}</p>
                <div className="flex items-center gap-1 mb-2">
                  <TrendingUp className="w-3 h-3 text-success" />
                  <span className="text-sm font-mono font-bold text-success">{score}%</span>
                </div>
                {a.streak > 0 && (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm border-2 ${LEVEL_COLORS[a.level]}`}>
                      {a.avatar}
                    </div>
                    ...
                    <span className="text-lg font-mono font-bold text-success">{score}%</span>
                  </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {a.kpis.leads < 8 && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-destructive/10 text-destructive border border-destructive/20">⚠ Leads baixo</span>
                    )}
                    {a.kpis.cadencia < 60 && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-destructive/10 text-destructive border border-destructive/20">⚠ Cadência fraca</span>
                    )}
                    {a.kpis.ligacoes < 20 && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-destructive/10 text-destructive border border-destructive/20">⚠ Poucas ligações</span>
                    )}
                    {a.kpis.reunioes < 3 && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-destructive/10 text-destructive border border-destructive/20">⚠ Reuniões abaixo</span>
                    )}
                  </div>
                </motion.div>
              );
